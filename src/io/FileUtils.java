...
public final class FileUtils
{
	...
	public static final FileNameExtensionFilter sourceFileFilter = new FileNameExtensionFilter("structerd files", "json");
	...
	private static final String DEFAULT_FILE_NAME = "scheduleResult.json";
	...
	public static void exportWithName(ExportableDocument exportableDocument)
	{
		...
		if(option == JFileChooser.APPROVE_OPTION)
		{
			...
			try
			{
				File file = new File(FILE_CHOOSER.getSelectedFile().getAbsolutePath() + getFileSeparator() + fileName + ".json");
				if(FileUtils.manageFileOnDisk(file))
				{
					exportableDocument.export(file);
					showExportCompletedDialog();
	...
	public static File askJSONFile()
	{
		FILE_CHOOSER = new JFileChooser();
		FILE_CHOOSER.setDialogTitle("Select JSON file");
		FILE_CHOOSER.setFileSelectionMode(JFileChooser.FILES_ONLY);
		FILE_CHOOSER.setCurrentDirectory(new File(DEFAULT_DIR_PATH));
		FILE_CHOOSER.setFileFilter(sourceFileFilter);
        int option = FILE_CHOOSER.showOpenDialog(null);
        if(option == JFileChooser.APPROVE_OPTION)
        {
